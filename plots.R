plot_digital <- function(Data, title, Pval_act, Pval_rep){
  ...
  for(f in 1:nrow(Data)){
    i = ((f-1)*4)+1:4
    X = rbind(X,i)
    TADplot.df.Act[i,"COUNT"] = c(Data$Proximal.Activated[f], Data$Proximal[f] - Data$Proximal.Activated[f],
                                  Data$Total.Activated[f] - Data$Proximal.Activated[f], (Data$Total[f]-Data$Proximal[f])-(Data$Total.Activated[f] - Data$Proximal.Activated[f]))
...
plot_locus <- function(Data){
  ...
  locus_plot <- ggplot(Data, aes(TSS)) +
    ...
    geom_rect(aes(fill = Regulation, xmin = TSS-5000, xmax = TSS+5000, ymin=-0.2, ymax=0.2), alpha=0.8) +
    geom_text_repel(
      y = 0,
      nudge_y      = -0.5,
      direction    = "x",
      ...
      vjust        = 0.2,
      segment.size = 0.2,
      label = ifelse(Data$Regulation=="lncRNA",Data$ENSEMBL,"")) +
